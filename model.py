...
from hamburger3d.burger3d import Hamburger3DBlock
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, normalize=True):
        ...
        if self.use_v2:
            x0 = self.layers1c[0](x0)
        x1 = self.layers1[0](x0)
        x1_out = self.proj_out(x1, normalize)
        if self.use_v2:
            x1 = self.layers2c[0](x1)
        x2 = self.layers2[0](x1)
        x2_out = self.proj_out(x2, normalize)
        if self.use_v2:
            x2 = self.layers3c[0](x2)
        x3 = self.layers3[0](x2)
        x3_out = self.proj_out(x3, normalize)
        if self.use_v2:
            x3 = self.layers4c[0](x3)
...
class HamLayer(nn.Module):
    """Wrapper to use Hamburger 3D blocks as a stage compatible with Swin BasicLayer API.

    It applies `Hamburger3DBlock`(s) and an optional downsample module (same as PatchMerging)
    following the same input/output shapes as `BasicLayer`.
    """
    def __init__(self, dim: int, depth: int, downsample: nn.Module | None = None, norm_layer: type[LayerNorm] = nn.LayerNorm, window_size: Sequence[int] | None = None) -> None:
        super().__init__()
        self.dim = dim
        self.depth = depth
        # create hamburger blocks with the same channel dimension
        self.ham_blocks = Hamburger3DBlock(in_channels=dim, num_blocks=depth)
        self.downsample = downsample
        self.window_size = window_size
        if callable(self.downsample):
            # instantiate like BasicLayer does
            self.downsample = downsample(dim=dim, norm_layer=norm_layer, spatial_dims=len(window_size) if window_size is not None else 3)

    def forward(self, x):
        x_shape = x.size()
        # Expecting input in (b, c, d, h, w) for 3D
        if len(x_shape) == 5:
            # apply hamburger blocks (they operate on b,c,d,h,w)
            x = self.ham_blocks(x)
            # apply downsample if provided; downsample expects (b, d, h, w, c)
            if self.downsample is not None:
                b, c, d, h, w = x.shape
                x = rearrange(x, "b c d h w -> b d h w c")
                x = self.downsample(x)
            ...
            # fallback for 2D inputs - pass through hamburgers if possible
            try:
                x = self.ham_blocks(x)
            except Exception:
                pass
